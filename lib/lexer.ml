...
let init =
  let checks =
    ...
    ; U.re_pair {|^print\b|} (fun _ -> Token.Print)
    ; U.re_pair {|^"([^"\\]|\\.)*"|} (fun x ->
...
let rec collect_helper lexer filepath source acc col =
  let source', skipped = skip_whitespace source in
  let col' = col + skipped in
  if source' = ""
  then acc
  else (
      ...
      | (fn, con) :: tail ->
        (match Re.exec_opt fn source' with
         | Some group ->
           ...
           let loc = { path = filepath; row = 0; col = col' } in
           let remaining =
             if source_len - len <= 0
             then ""
             else String.sub source' len (source_len - len)
           in
           collect_helper lexer filepath remaining ((loc, token) :: acc) (col' + len)
...
let collect lexer filepath source acc =
  collect_helper lexer filepath source acc 0 |> List.rev
;;
